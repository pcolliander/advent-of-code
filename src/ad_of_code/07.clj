...
(defn- read-and-map-input [input]
  (let [strings (str/split input #"\n")
        string-with-links (filter #(> (.indexOf % ",") 0) strings)
        top-programs (atom {})]
                                   
    (doall
      (for [s string-with-links]
        (swap! top-programs conj (hash-map (get-head s) (get-links s)))) )
  
    @top-programs))

(defn find-topmost-program [input]
  (let [top-programs (read-and-map-input input)]

    (for [k (keys top-programs)]
      (if (nil? (some #(when (= % k) %) (flatten (vals top-programs))))
        (println "result is: " k)) )))

(find-topmost-program input)
