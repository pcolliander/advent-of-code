...
[ [ 5048	177	5280	5058	4504	3805	5735	220	4362	1809	1521	230	772	1088	178	1794  ]
  ...
  [ 5149	2892	5854	2000  3995	5250	249	3916	184	2497	210	4601	3955	1110	5340    ]
...
(defn checksum-calc [input]
  (->> (to-array-2d input)
    (map calc-difference)
    (reduce +)))

;; (println (checksum-calc [[5,1,9,5], [7,5,3], [2,4,6,8]]))
;; (println (checksum-calc input))
...
(defn sum-of-each-row [input]
  (let [xf (comp 
             (map find-and-divide-even-divisible-numbers)
             (mapcat flatten)
             (remove nil?))]

    (transduce xf + (to-array-2d input))))
